...
def main():
    ...
    # ------------- Step 2. Reading the model (.xml and .bin files) -------------
    log.info("Reading the model files:\n\t{}\n\t{}".format(model_xml, model_bin))
    ...
    # ------------- 4. Preparing outputs info -------------
    yolo_layer_params = {}
    outputs = {}
    for output in model.outputs:
        output_shape = output.partial_shape.get_min_shape() if model.is_dynamic() else output.shape
        outputs[output.get_any_name()] = (output.get_names(), output_shape)
    
    for node in model.get_ordered_ops():
        layer_name = node.get_friendly_name()
        if layer_name not in outputs.keys():
            continue
        shape = list(outputs[layer_name][1])
        yolo_params = YoloParams(node.get_attributes(), shape[2:4], args.architecture_type)
    ...
    num_requests = 2
    async_queue = AsyncInferQueue(compiled_model, num_requests)
    if num_requests == 0:
        # +1 to use it as a buffer of the pipeline
        async_queue = AsyncInferQueue(compiled_model, len(async_queue) + 1)
    ...
    data_done = [False for _ in range(num_requests)]
    ...
    while cap.isOpened():
        ...
        #request.start_async({0: in_frame})
        async_queue.start_async({0: in_frame}, request_id)

        # Collecting object detection results
        objects = list()
        
        async_queue[cur_request_id].wait()
        det_time = time() - start_time

        if data_done[cur_request_id] == True:            
            start_time = time()
            for key in outputs.keys():
                layer_name = key
                layer_params = yolo_layer_params[layer_name]
                result = async_queue[cur_request_id].get_tensor(key).data[:]
                objects += parse_yolo_region(result, in_frame.shape[2:], \
                    frame.shape[:-1], layer_params[1], \
                    args.prob_threshold, args.architecture_type)
            data_done[cur_request_id] = False
